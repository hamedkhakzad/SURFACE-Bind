...
system = sys.argv[1]
site = sys.argv[2]
orig_pep_index = int(sys.argv[3]) - 1


all_dots = []
with open('out_peptides/{}/site_{}/target.vert'.format(system, site)) as f:
...
parent_dir = 'out_peptides/{}/site_{}'.format(system, site)

with open('analysis_fixed_size_site_{}/list_out_peptides_site_{}.txt'.format(site, site)) as f:
...
save_fragment('analysis_fixed_size_site_{}/out_pdb/{}_frag.pdb'.format(site, current_pep), orig_fragment)
...
np.save('analysis_fixed_size_site_{}/out_data/rmsd_{}'.format(site, orig_pep_index), all_rmsd)
